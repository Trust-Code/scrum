...
class ScrumSprint(models.Model):
    ...
    def _compute(self):
        for record in self:
            if record.date_start and record.date_stop:
                days = (date.today() - fields.Date.from_string(
                    record.date_start)).days
                record.progress = float(days) / float(record.time_cal()) * 100
                if date.today() >= fields.Date.from_string(record.date_stop):
                    record.date_duration = record.time_cal() * 9
                else:
                    record.date_duration = days * 9
            else:
                record.date_duration = 0

    def _compute_progress(self):
        for record in self:
            if record.planned_hours and record.effective_hours and \
               record.planned_hours != 0:
                record.progress = \
                    record.effective_hours / record.planned_hours * 100
            else:
                record.progress = 0
    ...
    def test_task(self, sprint):
        tags = self.env['project.category'].search([('name', '=', 'test')])
        if len(tags) == 0:
            tags.append(self.env['project.category'].create({'name': 'test'}))
        for tc in sprint.project_id.test_case_ids:
    ...
    date_start = fields.Date(string='Starting Date',
                             ...
                             readonly=True, states={'draft':
                                                    [('readonly', False)]})
    date_stop = fields.Date(string='Ending Date', readonly=True,
                            states={'draft': [('readonly', False)]})
    date_duration = fields.Integer(compute='_compute',
    ...
    project_id = fields.Many2one(
        ...
        required=True, help="If you have [?] in the project name, it means \
        there are no analytic account linked to this project.")
    product_owner_id = fields.Many2one(
        comodel_name='res.users', string='Product Owner', required=False,
        help="The person who is responsible for the product")
    scrum_master_id = fields.Many2one(
        comodel_name='res.users', string='Scrum Master', required=False,
    ...
    task_ids = fields.One2many(
        comodel_name='project.task', inverse_name='sprint_id')
...
<h1><ul>What was the goal of this sprint?</ul></h1><br/>
<h1><ul>Has the goal been reached?</ul></h1><br/><br/>
    """)
    retrospective = fields.Html(string='Sprint Retrospective', default="""
<h1><ul>What will you start doing in next sprint?</ul></h1>
<br/><br/>
<h1><ul>What will you stop doing in next sprint?</ul></h1>
<br/><br/>
<h1><ul>What will you continue doing in next sprint?</ul></h1>
    """)
    sequence = fields.Integer(
        'Sequence',
        help="Gives the sequence order when displaying a list of tasks.")
    progress = fields.Float(
        compute="_compute_progress", group_operator="avg", type='float',
        multi="progress", string='Progress (0-100)',
        help="Computed as: Time Spent / Total Time.")
    effective_hours = fields.Float(
        compute="_hours_get", multi="effective_hours",
        string='Effective hours',
        help="Computed using the sum of the task work done.")
    planned_hours = fields.Float(
        compute="_hours_get", multi="planned_hours", string='Planned Hours',
    ...
    def finish_sprint(self):
        for sprint in self:
            stage_cancelled = sprint.project_id.type_ids.filtered(
                lambda x: x.cancelled_state)
            if len(stage_cancelled) == 0:
                raise Warning(
                    u'Configure um estágio para as tarefas canceladas')
            stage_done = sprint.project_id.type_ids.filtered(
                lambda x: x.closed)
            if len(stage_done) == 0:
                raise Warning(
                    u'Configure um estágio para as tarefas concluídas')
                ...
                if task.stage_id.closed:
                    ...
                    if not task.stage_id.cancelled_state and \
                       self.env.user.company_id.cancel_open_tasks_scrum:
                        ...
                        task.sudo(user=sprint.project_id.user_id).stage_id = \
                            stage_cancelled[0].id
                        task.copy(default={'stage_id': stages[0].id,
                                           'name': task.name})
...
class ProjectUserStories(models.Model):
    ...
    actor_ids = fields.Many2many('project.scrum.actors', string='Actor')
    project_id = fields.Many2one(
        'project.project', string='Project', ondelete='set null',
        select=True, track_visibility='onchange', change_default=True)
    sprint_ids = fields.Many2many('project.scrum.sprint', string='Sprint')
    task_ids = fields.One2many('project.task', inverse_name='us_id')
    task_test_ids = fields.One2many('project.scrum.test', 'user_story_id_test')
    task_count = fields.Integer(compute='_task_count', store=True)
    test_ids = fields.One2many('project.scrum.test', 'user_story_id_test')
...
class ScrumMeeting(models.Model):
    ...
    project_id = fields.Many2one(
        ...
        select=True, track_visibility='onchange', change_default=True)
    name = fields.Char(
        string='Meeting', compute='_compute_meeting_name', size=60)
    ...
    user_id_meeting = fields.Many2one(
        'res.users', string='Name', required=True,
    ...
    question_backlog = fields.Selection(
        [('yes', 'Yes'), ('no', 'No')], string='Backlog Accurate?',
    ...
    def _compute_meeting_name(self):
        if self.project_id:
            self.name = "%s - %s - %s" % (self.project_id.name,
                                          self.user_id_meeting.name,
                                          self.date_meeting)
        else:
            self.name = "%s - %s" % (self.user_id_meeting.name,
                                     self.date_meeting)
...
class ProjectProject(models.Model):
    ...
    sprint_ids = fields.One2many(
        comodel_name="project.scrum.sprint", inverse_name="project_id",
        string="Sprints")
    user_story_ids = fields.One2many(
        comodel_name="project.scrum.us", inverse_name="project_id",
        string="User Stories")
    meeting_ids = fields.One2many(
        comodel_name="project.scrum.meeting", inverse_name="project_id",
        string="Meetings")
    test_case_ids = fields.One2many(
        comodel_name="project.scrum.test", inverse_name="project_id",
        string="Test Cases")
    sprint_count = fields.Integer(compute='_sprint_count',
                                  string="Sprints")
    user_story_count = fields.Integer(compute='_user_story_count',
                                      string="User Stories")
    meeting_count = fields.Integer(compute='_meeting_count',
                                   string="Meetings")
    test_case_count = fields.Integer(compute='_test_case_count',
                                     string="Test Cases")
    use_scrum = fields.Boolean(store=True)
    default_sprintduration = fields.Integer(
        string='Calendar', required=False, default=14,
        help="Default Sprint time for this project, in days")
    manhours = fields.Integer(
        string='Man Hours', required=False,
        help="How many hours you expect this project needs before it's \
        finished")
...
class TestCase(models.Model):
    ...
    sprint_id = fields.Many2one(comodel_name='project.scrum.sprint',
                                string='Sprint')
    user_story_id_test = fields.Many2one(comodel_name="project.scrum.us",
                                         string="User Story")
    ...
    stats_test = fields.Selection(
        [('draft', 'Draft'), ('in progress', 'In Progress'),
